...
const page = {
  ...
  heroesListAddButton: element.all(by.css('app-clusters > div button')).get(0),
  heroesListInput: element(by.css('app-clusters > div input')),
  heroesListSearchButton: element.all(by.css('app-clusters > div button')).get(1),
  heroesListItems: element.all(by.css('app-clusters ul li')),
...
xdescribe('Http Tests', function () {
  ...
  describe('Configuration', () => {
    it('can fetch the configuration JSON file', () => {
      ...
      expect(page.configSpan.getText()).toContain('Heroes API URL is "api/clusters"');
